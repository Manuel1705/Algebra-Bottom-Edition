...
\bottom{
    Sia \((s, \cdot)\) un semigruppo commutativo. Allora, \(\forall x, y \in s\), diremo che:
...
\subsection{ Insieme degli Associati}
\bottom{
    Sia \((s, \cdot)\) un semigruppo commutativo. Allora, \(\forall x \in s\), definiamo l'insieme degli associati di \(x\):
...
\bottomp{
    ...
    Allora $$\exists w,z\in s\text{ tali che }y = xw \text{ e } x = yz$$ Allora, usando la cancellabilità di \(x\):
...
\bottomp{
    ...
    \(2 \Leftrightarrow)\) Segue immediatamente dalla definizione di insieme dei divisori e dei multiplil.
}
...
\bottom{
    ...
    Cioè, \(p \in \mathbb Z\) primo \(\iff\) \(p\) irriducibile.
}
\bottomp{
...
\bottom{
    ...
    E la proiezione canonica \(\pi: x \in s \mapsto [x]_s \in s/\mathord\sim\) è epimorfismo tra le strutture \((s, *_1, \dots, *_n)\) e \((s/\mathord\sim, (*_1)_\sim, \dots, (*_n)_\sim)\)
}
...
\bottomp{
    ...
    \(2 \rightarrow 3)\) Siano \(a, b \in \mathbb Z: m = ab\). Allora $$[m]_m = [0]_m = [ab]_m = [a]_m \cdot [b]_m$$ Trovandoci in un dominio di integrità, vale la Legge di Annullamento del Prodotto e $$[a]_m \cdot [b]_m = [0]_m \iff [a]_m = [0]_m \lor [b]_m = [0]_m$$
...
\bottom{
    Siano \((g, \cdot)\) un gruppo ed $ x \in g$ un suo elemento periodico. $$\text{Il minimo } n \in \mathbb N - \{0\}: x^n = 1_g$$ si dice periodo di \(x\) e si indica \(|x|\).
}
